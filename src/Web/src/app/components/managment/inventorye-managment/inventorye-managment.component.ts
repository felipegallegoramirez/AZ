...
export class InventoryeManagmentComponent implements OnInit {
  ...
  constructor( private productCategoryService:ProductCategoryService, private inventoryService:InventoryService) { }
  ...
  search(){
    ...
    let or=order.split("/")
    this.inventoryService.getProductSearch(search,1,1000,or[0],Number(or[1]),cate).subscribe((res)=>{
      let data = res as Inventory[]
      this.products=[]
      for (let i=0; i< data.length ;i++){
        let ar=this.category.find((x)=>data[i].category==x.id)
  ...
  create_product(){
    ...
    let category= (<HTMLInputElement>document.getElementById("category_product")).value
    ...
    let file= (<HTMLInputElement>document.getElementById("file_product")).files?.item(0)
    let data = new Inventory(undefined,code,name,Number(count),category,Number(price),Number(points),undefined,undefined)
    delete data._id;
    delete data.image;
  ...
  update_product(){
    ...
    let name= (<HTMLInputElement>document.getElementById("name_product")).value
    let image= (<HTMLInputElement>document.getElementById("image_product")).value
    let category= (<HTMLInputElement>document.getElementById("category_product")).value
    let count= (<HTMLInputElement>document.getElementById("count_product")).value
    let code= (<HTMLInputElement>document.getElementById("code_product")).value
    let price= (<HTMLInputElement>document.getElementById("price_product")).value
    ...
    if(file){
      ...
      this.inventoryService.putInventoryI(data,file,idc).subscribe((res)=>{
        this.getProducts();
      })
    }else{
  ...
  delete_product(id:string){
    this.inventoryService.deleteInventory(id).subscribe((res)=>{this.getProducts();})
  }
  ...
  clear_product(){
    ...
    image.value=""
    let category= (<HTMLInputElement>document.getElementById("category_product"))
    category.value = ""
  ...
  create_category(){
    ...
    let file= (<HTMLInputElement>document.getElementById("file_category")).files?.item(0)
  ...
  edit_category(){
    ...
    if(file){
      this.productCategoryService.putProductCategoryI(data,id,file).subscribe((res)=>{
      })
    }else{
      this.productCategoryService.putProductCategory(data,id).subscribe((res)=>{
      })
    }
  }
  ...
  delete_category(){
    ...
    this.productCategoryService.deleteProductCategory(id).subscribe((res)=>{
      ...
      if (x.status){
        ...
        const lot  = this.category.findIndex(x=>{x.id===id})
        if(lot!= -1){
